...
export async function POST(request: Request) {
 ...
 const { data: row } = await supabase.from("moment_drama_pauses").insert({ user_id: user.id, ...parsed.data, ai_response: response }).select("id").single();
 const { data: aiMessage } = await supabase.from("moment_ai_messages").insert({ user_id: user.id, mode: "social_boundary_helper", source_table: "moment_drama_pauses", source_id: row?.id, input_snapshot: parsed.data, output_snapshot: response, safety_flags: risk.flags }).select("id").single();
 return NextResponse.json({ response, persisted: { dramaPauseId: row?.id ?? null, aiMessageId: aiMessage?.id ?? null } });
}
