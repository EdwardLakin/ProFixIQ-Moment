...
export default async function JournalPage(){
  ...
  return <MomentAppShell title="Journal" subtitle="Private storage for your Moment history.">
    ...
    <section className="mt-4 space-y-4"><div className="text-xs text-slate-300">Filters: All / Manual / Moments</div>{(rows??[]).length===0?<p className="rounded-xl bg-white/[0.03] p-4 text-slate-300">No entries yet. Your conversations and manual notes will appear here.</p>:Array.from(byDate.entries()).map(([date, entries])=><article key={date} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><h3 className="text-sm font-medium">{date}</h3><ul className="mt-2 space-y-2">{entries?.map((entry)=><li key={entry.id} className="rounded-xl bg-black/20 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs uppercase text-slate-400">{entry.source === "manual" ? "Manual" : "Moment"}</p><form action={deleteJournalEntry}><input type="hidden" name="entryId" value={entry.id} /><button type="submit" className="text-xs text-slate-300 underline hover:text-white">Archive</button></form></div><p className="text-sm text-slate-100">{entry.title ?? entry.input_summary}</p>{entry.content ? <p className="mt-1 text-sm text-slate-300 whitespace-pre-wrap">{entry.content}</p> : null}</li>)}</ul></article>)}</section>
  </MomentAppShell>
}
