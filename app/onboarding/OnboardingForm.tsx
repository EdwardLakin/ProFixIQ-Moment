...
const teenChips = [["school_overwhelm", "School feels overwhelming"], ["task_start", "Starting is hard"], ["math_stress", "Math gets frustrating"], ["social_pressure", "Friend drama pulls me in"], ["shutdown", "I shut down when confused"], ["calm_restart", "I want calmer restarts"]];
const adultChips = [["work_stress", "Work stress"], ["money_unclear", "Money feels unclear"], ["relationship_stress", "Relationship stress"], ["household_overload", "Household overload"], ["life_admin", "Life admin"], ["decision_fatigue", "Decision fatigue"], ["task_start", "Starting is hard"], ["calm_restart", "I want calmer restarts"]];
...
export function OnboardingForm({ action }: { action: CompleteOnboardingAction }) {
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <fieldset><p className="mb-3 text-sm font-medium text-slate-100">What tends to feel heavy?</p><div className="grid gap-3 sm:grid-cols-2">{chips.map(([value, label]) => <label key={value} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.07] px-4 py-3 text-sm text-slate-100"><input type="checkbox" name="focus_areas" value={value} className="h-4 w-4" />{label}</label>)}</div></fieldset>
    
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
      <p className="text-sm text-slate-100">Parent/guardian email (optional)</p>
      <p className="mt-1 text-xs text-slate-300">Only used for optional guardian summaries. Private entries are not shared.</p>
