...
function getResponseDepth(input: RouteMomentInput, profile: PacingProfile): ResponseDepth { const normalized = `${input.momentText} ${input.selectedSignals.join(" ")}`.toLowerCase(); if (profile === "grief" || profile === "shutdown" || normalized.includes("can't do") || normalized.includes("cannot do")) return "heavy"; if (profile === "overwhelm" || normalized.includes("exhaust") || normalized.includes("numb") || normalized.includes("spiral")) return "overwhelmed"; return "light"; }
...
function composeNaturalResponse(domain: ReturnType<typeof classifyResponseDomain>, capabilityWeights: ReturnType<typeof buildUnifiedCognition>["capabilityWeights"], momentText: string, opts: { activeDomain: string; suppressedDomains: string[]; emotionalCarryoverStrength: number }) {
  ...
  if (domain === "tutor") {
    ...
    return {
      reflection: warmth,
      tinyNextStep: "What part is mixing you up most right now?",
      supportiveNote: "",
    };
  }
  ...
  if (domain === "grief_loss") {
    return {
      ...
      tinyNextStep: "You don’t have to turn this into a task. We can just stay with what this brought up.",
      supportiveNote: "",
    };
  }
  ...
  if (domain === "conflict") {
    return {
      ...
      tinyNextStep: "Do you want help turning what you feel into a calmer message, or do you just want to talk through the hurt first?",
      supportiveNote: "",
    };
  }
  ...
  return {
    ...
    tinyNextStep: "What part feels most present right now?",
    supportiveNote: "",
  };
}
...
export function runMomentOrchestrator(input: RouteMomentInput): MomentOrchestratorResult {
  ...
  return {
    ...
    response: {
      ...
      followUpActions: domainBehavior.offerTool ? [{ label: "Show gentle options", href: route.routePath }] : [],
      blocks,
    },
