...
export function ModuleFlowClient({ searchParams, apiPath, fields, title }: { searchParams?: { from?: string; contextId?: string; summary?: string }; apiPath: string; fields: Array<{ name: string; label: string; area?: boolean }>; title: string }) {
  ...
  return <div className="space-y-4"><CarriedContextCard searchParams={searchParams} /><MomentCard><form action={onSubmit} className="space-y-3">{fields.map((field) => field.area ? <MomentTextarea key={field.name} name={field.name} required={field.name.includes("text")} placeholder={field.label} defaultValue={field.name.includes("text") ? (searchParams?.summary ?? "") : ""} /> : <MomentInput key={field.name} name={field.name} placeholder={field.label} />)}<MomentButton type="submit">Get guided reset</MomentButton></form></MomentCard>{result ? <MomentCard><div className="space-y-3">{result.blocks?.map((block, i) => block.type === "momentum_builder" ? <MomentumBuilderBlock key={i} text={block.text} /> : block.type === "route_transition" ? <RouteTransitionBlock key={i} text={block.text} /> : <RecoveryPromptBlock key={i} text={block.text} />)}</div><Link href="/dashboard" className="mt-4 inline-flex rounded-full border border-white/15 px-4 py-2 text-sm">Return to Moment</Link></MomentCard> : null}</div>;
}
